...
public class ConfigParserTest extends CaseInitializer {
	...
	@Before
	public void setUp() {
		String path = ConfigParserTest.class.getClassLoader().getResource(".").getFile();
		this.jobPath = path + File.separator + "job" + File.separator + "job.json";
	}
	...
	private Map<String, String> getPublicKeyMap() {
		Map<String, String> versionKeyMap = new HashMap<String, String>();
		...
		String lastKeyVersion = properties.getProperty(CoreConstant.LAST_KEYVERSION);
		String lastPublicKey = properties.getProperty(CoreConstant.LAST_PUBLICKEY);
		String lastPrivateKey = properties.getProperty(CoreConstant.LAST_PRIVATEKEY);
		if (StringUtils.isNotBlank(lastKeyVersion)) {
			if (StringUtils.isBlank(lastPublicKey) || StringUtils.isBlank(lastPrivateKey)) {
		...
		String currentKeyVersion = properties.getProperty(CoreConstant.CURRENT_KEYVERSION);
		String currentPublicKey = properties.getProperty(CoreConstant.CURRENT_PUBLICKEY);
		String currentPrivateKey = properties.getProperty(CoreConstant.CURRENT_PRIVATEKEY);
		if (StringUtils.isNotBlank(currentKeyVersion)) {
			if (StringUtils.isBlank(currentPublicKey) || StringUtils.isBlank(currentPrivateKey)) {
				throw DataXException.asDataXException(FrameworkErrorCode.SECRET_ERROR,
		...
		if (versionKeyMap.size() <= 0) {
			throw DataXException.asDataXException(FrameworkErrorCode.SECRET_ERROR, "DataX配置要求加解密，但无法找到公私钥");
		}
