...
public class FtpWriter extends Writer {
	public static class Job extends Writer.Job {
		...
		public void init() {
			...
			try {
				...
				String message = String.format("与ftp服务器建立连接失败, host:%s, username:%s, port:%s, errorMessage:%s", host,
		...
		private void validateParameter() {
			this.writerSliceConfig.getNecessaryValue(com.alibaba.datax.plugin.unstructuredstorage.writer.Key.FILE_NAME,
			...
			if (!path.startsWith("/")) {
				...
				LOG.error(message);
				throw DataXException.asDataXException(FtpWriterErrorCode.ILLEGAL_VALUE, message);
			}

			this.host = this.writerSliceConfig.getNecessaryValue(Key.HOST, FtpWriterErrorCode.REQUIRED_VALUE);
			this.username = this.writerSliceConfig.getNecessaryValue(Key.USERNAME, FtpWriterErrorCode.REQUIRED_VALUE);
			this.password = this.writerSliceConfig.getNecessaryValue(Key.PASSWORD, FtpWriterErrorCode.REQUIRED_VALUE);
			this.timeout = this.writerSliceConfig.getInt(Key.TIMEOUT, Constant.DEFAULT_TIMEOUT);

			this.protocol = this.writerSliceConfig.getNecessaryValue(Key.PROTOCOL, FtpWriterErrorCode.REQUIRED_VALUE);
			if ("sftp".equalsIgnoreCase(this.protocol)) {
				...
				this.port = this.writerSliceConfig.getInt(Key.PORT, Constant.DEFAULT_FTP_PORT);
				this.ftpHelper = new StandardFtpHelperImpl();
			} else {
				throw DataXException.asDataXException(FtpWriterErrorCode.ILLEGAL_VALUE,
		...
		public void prepare() {
			...
			if ("truncate".equals(writeMode)) {
				...
				for (String each : allFileExists) {
					fullFileNameToDelete.add(UnstructuredStorageWriterUtil.buildFilePath(path, each, null));
				}
				LOG.info(String.format("删除目录path:[%s] 下指定前缀fileName:[%s] 文件列表如下: [%s]", path, fileName,
				...
				LOG.info(String.format("由于您配置了writeMode append, 写入前不做清理工作, [%s] 目录下写入相应文件名前缀  [%s] 的文件", path,
						fileName));
				LOG.info(String.format("目录path:[%s] 下已经存在的指定前缀fileName:[%s] 文件列表如下: [%s]", path, fileName,
				...
				if (!allFileExists.isEmpty()) {
					LOG.info(String.format("目录path:[%s] 下指定前缀fileName:[%s] 冲突文件列表如下: [%s]", path, fileName,
							StringUtils.join(allFileExists.iterator(), ", ")));
					throw DataXException.asDataXException(FtpWriterErrorCode.ILLEGAL_VALUE,
							String.format("您配置的path: [%s] 目录不为空, 下面存在其他文件或文件夹.", path));
				}
			} else {
				throw DataXException.asDataXException(FtpWriterErrorCode.ILLEGAL_VALUE, String
		...
		public void destroy() {
			try {
				...
				String message = String.format("关闭与ftp服务器连接失败, host:%s, username:%s, port:%s, errorMessage:%s", host,
						username, port, e.getMessage());
				LOG.error(message, e);
			}
		}
	...
	public static class Task extends Writer.Task {
		...
		public void startWrite(RecordReceiver lineReceiver) {
			...
			String fileFullPath = UnstructuredStorageWriterUtil.buildFilePath(this.path, this.fileName, this.suffix);
			...
			try {
				...
				throw DataXException.asDataXException(FtpWriterErrorCode.WRITE_FILE_IO_ERROR,
