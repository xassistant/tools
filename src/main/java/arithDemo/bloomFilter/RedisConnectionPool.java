...
public class RedisConnectionPool {
    ...
    private static void initPool() {
        try {
            Properties config = LoadFileUtils.loadProperties(EnvKit.isWin() ?
                    ConfigFileName.RedisDev.getValue() : ConfigFileName.Redis.getValue());

            String intelligenceHost = config.getProperty("intelligenceHost");
            int intelligencePort = Integer.parseInt(config.getProperty("intelligencePort"));
...
//            loadPoolCache(RedisAddress.MlNormalization.getValue(), mlNormalizationPool);
            ...
            logger.error("initPool error: " + e.getMessage());
        }
    }
    ...
    private static JedisPoolConfig getJedisPoolConfig(Properties config) {
        ...
        jedisPoolConfig.setMaxTotal(Integer.parseInt(config.getProperty("maxActive")));
        jedisPoolConfig.setMaxIdle(Integer.parseInt(config.getProperty("maxIdle")));
        jedisPoolConfig.setMaxWaitMillis(Integer.parseInt(config.getProperty("maxWait")));
        jedisPoolConfig.setTestOnBorrow(Boolean.parseBoolean(config.getProperty("testOnBorrow")));
        jedisPoolConfig.setTestOnReturn(Boolean.parseBoolean(config.getProperty("testOnReturn")));
     ...
     */
    public static boolean updateRedisOutputResource(String ruleOutputHost, int ruleOutputPort, String ruleOutputPassword,
                                                    String esOutputHost, int esOutputPort, String esOutputPassword) {
        try {
            Properties config = LoadFileUtils.loadProperties(EnvKit.isWin() ?
                    ConfigFileName.RedisDev.getValue() : ConfigFileName.Redis.getValue());
            ...
            return LoadFileUtils.updateProperties(config, EnvKit.isWin() ?
                    ConfigFileName.RedisDev.getValue() : ConfigFileName.Redis.getValue());
