...
public class Configuration {
	...
	private void setObject(final String path, final Object object) {
		...
		throw DataXException.asDataXException(CommonErrorCode.RUNTIME_ERROR, String.format(
				"值[%s]无法适配您提供[%s]， 该异常代表系统编程错误, 请联系开发团队!", ToStringBuilder.reflectionToString(object), path));
	}
	...
	public Object buildObject(final List<String> paths, final Object object) {
		if (null == paths) {
			throw DataXException.asDataXException(CommonErrorCode.RUNTIME_ERROR,
					"Path不能为null，该异常代表系统编程错误, 请联系开发团队 !");
		}
		...
		for (int i = paths.size() - 1; i >= 0; i--) {
			...
			throw DataXException.asDataXException(CommonErrorCode.RUNTIME_ERROR, String
					.format("路径[%s]出现非法值类型[%s]，该异常代表系统编程错误, 请联系开发团队! .", StringUtils.join(paths, "."), path));
		}
	...
	@SuppressWarnings("unchecked")
	public Object setObjectRecursive(Object current, final List<String> paths, int index, final Object value) {
		...
		throw DataXException.asDataXException(CommonErrorCode.RUNTIME_ERROR, "该异常代表系统编程错误, 请联系开发团队 !");
	}
	...
	private Object findObjectInList(final Object target, final String each) {
		...
		if (!StringUtils.isNumeric(index)) {
			throw new IllegalArgumentException(
					String.format("系统编程错误，列表下标必须为数字类型，但该节点发现实际类型是[%s] ，该异常代表系统编程错误, 请联系开发团队 !", index));
		}
	...
	private void checkPath(final String path) {
		if (null == path) {
			throw new IllegalArgumentException("系统编程错误, 该异常代表系统编程错误, 请联系开发团队!.");
		}
