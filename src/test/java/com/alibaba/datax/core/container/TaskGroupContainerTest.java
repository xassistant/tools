...
public class TaskGroupContainerTest extends CaseInitializer {
	...
	@Before
	public void setUp() {
		String path = TaskGroupContainerTest.class.getClassLoader().getResource(".").getFile();
		...
		this.configurationFakeTransformer = ConfigParser.parse(path + File.separator + "allHasFakeTransformer.json");
		initConfiguration(this.configurationFakeTransformer);
		this.configurationRealTransformer = ConfigParser.parse(path + File.separator + "allHasRealTransformer.json");
	...
	public void testLongTimeWriter() {
		...
		try {
			Method com = TaskGroupContainer.class.getDeclaredMethod("reportTaskGroupCommunication", Communication.class,
					int.class);
	...
	public void testFakeTransformer() {
		...
		Configuration jobContent = this.configurationFakeTransformer
	...
	public void testRealTransformer() {
		...
		Configuration jobContent = this.configurationRealTransformer
				.getListConfiguration(CoreConstant.DATAX_JOB_CONTENT).get(0);
		List<Configuration> jobContents = new ArrayList<Configuration>();
		jobContents.add(jobContent);
