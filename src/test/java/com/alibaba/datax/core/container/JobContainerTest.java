...
public class JobContainerTest extends CaseInitializer {
	...
	public void setUp() {
		...
		this.configuration = ConfigParser.parse(path + File.separator + "all0.json");
	...
	public void testInitNormal() throws Exception {
		...
		initMethod.setAccessible(true);
		initMethod.invoke(jobContainer, new Object[] {});
		Assert.assertEquals("default job id = 0", 0l,
				this.configuration.getLong(CoreConstant.DATAX_CORE_CONTAINER_JOB_ID).longValue());
	}
	...
	public void testMergeReaderAndWriterSlicesConfigs() throws Exception {
		...
		initMethod = jobContainer.getClass().getDeclaredMethod("mergeReaderAndWriterTaskConfigs", List.class,
				List.class);
		initMethod.setAccessible(true);

		List<Configuration> mergedConfigs = (List<Configuration>) initMethod.invoke(jobContainer,
	...
	public void testMergeReaderAndWriterSlicesConfigsException() throws Exception {
		...
		initMethod = jobContainer.getClass().getDeclaredMethod("mergeReaderAndWriterSlicesConfigs", List.class,
				List.class);
		initMethod.setAccessible(true);
	 ...
	 */
	@SuppressWarnings("unchecked")
	private void distributeTasksToTaskGroupContainerTest(int channelNumber, int channelsPerTaskGroupContainer)
			throws Exception {
		JobContainer jobContainer = new JobContainer(this.configuration);
		Method initMethod = jobContainer.getClass().getDeclaredMethod("init");
		initMethod.setAccessible(true);
		...
		initMethod = jobContainer.getClass().getDeclaredMethod("distributeTasksToTaskGroup", int.class, int.class,
				int.class);
		initMethod.setAccessible(true);
		List<Configuration> taskGroupConfigs = (List<Configuration>) initMethod.invoke(jobContainer,
				averSlicesPerChannel, channelNumber, channelsPerTaskGroupContainer);
		initMethod.setAccessible(false);

		Assert.assertEquals("task size check", channelNumber / channelsPerTaskGroupContainer
				+ (channelNumber % channelsPerTaskGroupContainer > 0 ? 1 : 0), taskGroupConfigs.size());
		int sumSlices = 0;
		for (Configuration taskGroupConfig : taskGroupConfigs) {
			Assert.assertNotNull("have set taskGroupId",
					taskGroupConfig.getInt(CoreConstant.DATAX_CORE_CONTAINER_TASKGROUP_ID));
	...
	public void testErrorLimitIgnoreCheck() throws Exception {
		...
		// LocalTaskGroupCommunicationManager.updateTaskGroupCommunication(0,
		// communication);
	...
	public void testErrorLimitPercentCheck() throws Exception {
		...
		// this.configuration.set(CoreConstant.DATAX_JOB_SETTING_ERRORLIMIT_RECORD,
		// null);
